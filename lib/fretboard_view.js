...
class FretboardView {
	...
	draw_background(){
		...
		ctx.beginPath();
		ctx.lineWidth = 6;
		ctx.strokeStyle = this.darkMode ? "#f5f5f5" :"#000";
	...
	drawHoverNote(note) {
		...
		this.drawNoteWithColor(note, label, "#eee", canvas);
	}

	drawNotePlaceholder(note, label) {
		this.drawNoteWithColor(note, label, "#ddd", this.canvas);
	}
		...
		for(i=range.min; i<=range.max; i++){
			...
			for(j=0; j<note_array.length; j++) {
				...
				if(note2.midi_value == note1.midi_value) {
					var label = withLabels ? chord.note_labels[j] : undefined;
					if (label == 'R'){
	...
	drawScale(scale) {
		...
		for(j=0; j<note_array.length; j++) {
			...
			var label = this.show_labels ? scale.getLabel(note) : undefined;
			if (label == 'R'){
